...
public class GcsFileUploader implements FileUploader {
    ...
    public String upload(FileTag tag, FileInfo.FileSource source) {
        ...
        try {
            ...
            log.info("GCS 파일 업로드 성공 - Bucket: {}, Path: {}", properties.bucket(), relativePath);

            return relativePath;
        } catch (IOException e) {
            log.error("GCS 파일 업로드 중 시스템 오류 발생: {}", e.getMessage(), e);
            throw new FileStorageException("파일 저장소(GCS) 업로드에 실패하였습니다.");
        }
